...
private:
    ...
    // the owning unique_ptr slot (not a raw pointer) so a node can
    // be *replaced* with a brand new one -- e.g. an entire
    // BinaryExpression(BitXor, x, x) subtree collapsing into a
    // single ImmValue(0).
    void simplify_expr(std::unique_ptr<Expression>& expr) {
        ...
        if (auto* binary = dynamic_cast<BinaryExpression*>(expr.get())) {
            ...
            // Simplify children first (bottom-up), so nested
            // self-xors fold before we check this node.
            ...
            // Rule 2: X + (-C) -> X - C
            //
            // ImmValue::value is stored as a raw uint64_t, so a
            // "negative" immediate is one whose bit pattern, read
            // as signed 64-bit two's complement, is < 0 (e.g. the
            // Lifter/Capstone hands us "add rsp, -8" as
            // Add(rsp, 0xFFFFFFFFFFFFFFF8)). Rewriting the operation
            // to Sub and negating the immediate back to its
            // positive magnitude keeps the value identical while
            // making the IR read the way a human would write it.
            if (binary->operation() == Operation::Add) {
                ...
                if (auto* imm = dynamic_cast<ImmValue*>(binary->right.get())) {
                    ...
                    // INT64_MIN has no positive two's-complement
                    // counterpart (negating it overflows), so it's
                    // deliberately left alone.
                    if (signed_value < 0 &&
                        signed_value != std::numeric_limits<int64_t>::min()) {

                        binary->op = Operation::Sub;
                        binary->right =
                            std::make_unique<ImmValue>(-signed_value);
                    }
                }
            }
        ...
        // Value leaves (register / immediate / memory) have
        // nothing further to simplify.
    }

    // Structural equality between two operands of a XOR, used to
    // detect "X ^ X". Only Value-kind nodes are compared (register,
    // immediate, memory)
    ...
    bool same_value(Expression* a, Expression* b) const {
        ...
        if (auto* ma = dynamic_cast<MemoryValue*>(va)) {
            ...
            if (ma->size != mb->size ||
                ma->scale != mb->scale ||
                ma->displacement != mb->displacement)
                return false;
